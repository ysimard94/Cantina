    ...
    <div>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8 h-full">
            <div class="bg-bg_rose flex flex-col overflow-hidden shadow rounded-lg py-2 m-2 h-full"
                v-for="bouteille in paginatedBouteilles" :key="bouteille.id">
                <img :src="bouteille.photo" :alt="bouteille.nom" class="w-1/3 mx-auto" />
                <div class="px-4 pt-4 flex flex-col justify-between">
                    <h4 class="text-base font-semibold text-vin_rouge text-left h-[67px] leading-tight">{{ bouteille.nom }}
                    </h4>
                    <div class="mt-4 flex justify-between items-center">
                        ...
                        <div>
                            <div class="text-black font-medium text-right">Note </div>
                            <div class="flex items-center">
                                <div class="text-gray-400 font-medium text-right">{{ bouteille.note }}%({{
                                    bouteille.nbr_notes }} avis)</div>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
 ...
 async ajouterAuCellier(bouteille) {
        try {
           const response= await BouteilleCellierDataServive.ajouterBouteilleAuCellier(bouteille.id,1)
