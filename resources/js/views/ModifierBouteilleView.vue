        ...
        <div class="bg-bg-rose m-4 p-3 shadow-md rounded">
            <form @submit.prevent="modifierBouteille" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input v-model="nom" id="nom" class="w-full rounded pt-2 pb-2 pl-1 pr-1" :class="{
                ...
                <div class="mb-4">
                    <label for="description" class="block text-lg text-left font-bold text-vin-rouge">Description</label>
                    <textarea v-model="description" id="description" class="w-full rounded pt-2 pb-2 pl-1 pr-1"></textarea>
                </div>
                <!-- add a phot here -->
                <div class="mb-4">
                    <label for="photo" class="block text-lg text-left font-bold text-vin-rouge">Select an image:</label>
                    <input @change="chargerPhoto" id="photo" type="file" accept="image/*" name="photo"
                        class="w-full rounded pt-2 pb-2 pl-1 pr-1" />
                </div>

                <div class="mb-4">
                    <label for="pays_id" class="block text-lg text-left font-bold text-vin-rouge">
                        Pays
                    </label>
                    <select v-model="pays_id" id="pays_id" class="w-full rounded pt-2 pb-2 pl-1 pr-1" :class="{
                            'border border-red-500':
                    ...
                    <p v-if="v$.pays_id.$error" class="block text-md text-red-500">
                        Veillez sélectionner un pays
                    </p>
                </div>
                <div class="mb-4">
                    <label for="categorie_id" class="block text-lg text-left font-bold text-vin-rouge">
                        Catégorie
                    </label>
                    <select v-model="categorie_id" id="categorie_id" class="w-full rounded pt-2 pb-2 pl-1 pr-1" :class="{
                            'border border-red-500':
                    ...
                    <p v-if="v$.categorie_id.$error" class="block text-md text-red-500">
                        Veillez sélectionner une catégorie
                    </p>
                </div>
                <div class="mb-4">
                    <label for="annee" class="block text-lg text-left font-bold text-vin-rouge">Année</label>
                    <input v-model="annee" id="annee" class="w-full rounded pt-2 pb-2 pl-1 pr-1" :class="{
                            'border border-red-500':
                                v$.annee.$error && v$.annee.$dirty,
                            'border border-green-500':
                                !v$.annee.$error && v$.annee.$dirty,
                        }" />
                    <p v-if="v$.annee.$error" class="block text-md text-red-500">
                        Veillez entrer une année valide
                    </p>
                </div>

                <div class="mb-4">
                    <label for="annee" class="block text-lg text-left font-bold text-vin-rouge">Note (%)</label>
                    <input v-model="note" id="annee" class="w-full rounded pt-2 pb-2 pl-1 pr-1" :class="{
                            'border border-red-500':
                ...
                <div class="mb-4">
                    <label for="annee" class="block text-lg text-left font-bold text-vin-rouge">prix</label>
                    <input v-model="prix" id="annee" class="w-full rounded pt-2 pb-2 pl-1 pr-1" :class="{
                            'border border-red-500':
...
import { required, minLength, integer, helpers, numeric, } from "@vuelidate/validators";
