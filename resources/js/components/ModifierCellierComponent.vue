    ...
    <div class="flex flex-col items-start space-y-4">
        ...
        <form @submit.prevent="modifierCellier" class="flex items-center w-full">
            ...
            <div class="flex items-center ml-auto">
                ...
                <button v-if="celliers.length > 1" type="button"
                    class="material-symbols-outlined w-9 py-1 font-semibold text-vin-rouge" @click="supprimerCellier">
                    delete
                </button>
            </div>
        </form>
    ...
    methods: {
        async modifierCellier () {
            ...
            try {
                ...
                const response = await CellierDataService.modifier(
                    this.cellier.id,
                    cellier
                );
        ...
        async supprimerCellier () {
            try {
                const response = await CellierDataService.supprimer(
                    this.cellier.id
                );
